...
try:
    ...
    # Fallback: define a minimal ToTensorV2 substitute
    class ToTensorV2:
        def __init__(self):
            pass
        def __call__(self, **kwargs):
            ...
            out = {'image': img}
            if 'bboxes' in kwargs:
                out['bboxes'] = kwargs['bboxes']
            if 'cls_labels' in kwargs:
                out['cls_labels'] = kwargs['cls_labels']
            return out
...
class VocDetectorDataset(DataLoader.Dataset):
    ...
    def encoder(self, image, boxes:list, labels:list):
        ...
        assert len(boxes) == int(sum([target_obj[scale].sum() for scale in range(num_scales)])), f"Some boxes were not assigned to any scale. {len(boxes)} vs {[int(target_obj[scale].sum()) for scale in range(num_scales)]}"
